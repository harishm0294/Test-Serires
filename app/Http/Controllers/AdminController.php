...
class AdminController extends Controller
{
    ...
    {
        $studentsCount = User::orderBy('created_at', 'DESC')
                ->get()->count();
        $examCount = Aexam::orderBy('created_at', 'DESC')
                ->get()->count();              
        return view('admin',compact('studentsCount', 'examCount'));
    }
    ...
    public function Get_Full_Detail_Result(Request $req) {
        ...
        ->where(
            [
                'exam_question.examcode' => $req->examcode,
                'result.student_id' => $req->userid
            ]
        )
        ->get();
    ...
    public function Update_Students_Result_List(Request $req) {
        ...
        ->where(['exam.category'   => $req->val])
        ->get();
        return response()->json(array('exam' => $result));
    }
   
    public function addquest ($id, $title, $tname, $cat, $time) {
        $question = DB::table('exam_question')->orderBy('created_at', 'ASC')->where('examcode',$id)
        ->get();
        $subject =  DB::table('exam_subject')->orderBy('created_at', 'DESC')->where('examcode',$id)
        ->get();
